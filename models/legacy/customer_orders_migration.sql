...
from {{ source("jaffle_shop_classic", "orders")}} as orders

join (
      select 
        first_name || ' ' || last_name as name, 
        * 
      from {{ source("jaffle_shop_classic", "customers")}}
) customers
...
join (
    ...
    from (
      select 
        ...
        *
      from {{ source("jaffle_shop_classic", "orders")}}
    ) a
    ...
    left outer join {{ source("stripe_classic", "payment")}} c
    on a.id = c.orderid
...
left outer join {{ source("stripe_classic", "payment")}} payments
